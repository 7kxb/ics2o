...
# global variables
player_x = (SCREEN_WIDTH / 2) / 2 + 160
player_y = (SCREEN_WIDTH / 2) / 2 + 0
...
if os.name == 'nt':
    ...
    windll.user32.MoveWindow(hwnd,0,0,SCREEN_WIDTH,SCREEN_HEIGHT,False)
...
while True:
    ...
    if int(fps) > 0 and toggle:
        toggle = False
        os.system("start ./old/fourthwall.vbs")
    elif player_x > (SCREEN_WIDTH / 2) / 2 and player_y > (SCREEN_WIDTH / 2) / 2 + 60:
        pygame.quit()
        os.system("python ./dui/scene7.py")
    elif player_x > (SCREEN_WIDTH / 2) / 2 and player_y < (SCREEN_WIDTH / 2) / 2 - 60:
        pygame.quit()
        os.system("python ./dui/scene7.py")